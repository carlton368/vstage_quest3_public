...
{
    internal class Mirror_2020_2_Editor : BaseShaderGUI
    {
        ...
        public override void OnGUI(MaterialEditor materialEditorIn, MaterialProperty[] properties)
        {
            ...
            MaterialProperty _EnableLocallyCorrection = ShaderGUI.FindProperty("_EnableLocallyCorrection", properties);

            MenuLocallyCorrection = EditorGUILayout.BeginFoldoutHeaderGroup(MenuLocallyCorrection, new GUIContent { text = "Locally Correction" });

            bool enableLCRS = false;
            if (MenuLocallyCorrection)
            {
                enableLCRS = _EnableLocallyCorrection.floatValue != 0.0f;
                enableLCRS = EditorGUILayout.Toggle("Enable", enableLCRS);
                _EnableLocallyCorrection.floatValue = enableLCRS ? 1.0f : 0.0f;

                //if (realTimeRef)
                //{
                //    MaterialProperty _EnviCubeIntensity = ShaderGUI.FindProperty("_EnviCubeIntensity", properties);
                //    materialEditor.ShaderProperty(_EnviCubeIntensity, "Intensity");

                //    MaterialProperty _EnviCubeSmoothness = ShaderGUI.FindProperty("_EnviCubeSmoothness", properties);
                //    materialEditor.ShaderProperty(_EnviCubeSmoothness, "Smoothness");
                //}
            }
